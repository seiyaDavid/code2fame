...
def optimize_model_parameters(model_type, data, metadata, n_trials=10):
    ...
    def objective(trial):
        ...
        if model_type in ["CTGAN", "CopulaGAN"]:
            ...
            # Also optimize PAC parameter
            pac = trial.suggest_categorical("pac", [10, 20, 30])
        else:
        ...
        if model_type in ["CTGAN", "CopulaGAN"]:
            ...
            if model_type == "CTGAN":
                model = CTGANSynthesizer(
                    ...
                    pac=pac,  # Use the optimized PAC value
                    verbose=False,
                )
            else:  # CopulaGAN
            ...
            # L2 regularization for TVAE
            l2_regularization = trial.suggest_float("l2_regularization", 0.0, 0.2)
...
def initialize_and_train_model(
    ...
    try:
        ...
        status_text.text(f"Error checking metadata: {str(e)}. Creating new metadata...")
        metadata = create_metadata_from_data(data)
        if metadata is None:
            status_text.text("Failed to create metadata. Aborting.")
            return None
    ...
    if optimize:
        status_text.text("Optimizing model parameters...")
        progress_bar.progress(20)

        # Run optimization
        optimized_params = optimize_model_parameters(
            model_type, data, metadata, n_trials=n_trials
        )

        # Update params with optimized values
        params.update(optimized_params)

        status_text.text(f"Optimization complete. Best parameters: {optimized_params}")
        progress_bar.progress(40)

    # Process parameters
    # Convert string layer dimensions to lists of integers
    if model_type in ["CTGAN", "CopulaGAN"]:
        ...
        if "discriminator_dim" in params and isinstance(
            params["discriminator_dim"], str
        ):
    ...
    # Ensure numeric parameters are the right type
    for key in ["epochs", "batch_size", "embedding_dim"]:
        if key in params and isinstance(params[key], str):
            try:
                params[key] = int(params[key])
            except ValueError:
                status_text.text(
                    f"Invalid value for {key}: {params[key]}. Using default."
                )
                params.pop(key)

    try:
        # Initialize the appropriate model with ALL parameters from optimization
        if model_type == "CTGAN":
            model = CTGANSynthesizer(
                metadata,
                ...
                # Only use default pac if not provided by optimization
                pac=params.get("pac", 10),
                verbose=True,
            )
